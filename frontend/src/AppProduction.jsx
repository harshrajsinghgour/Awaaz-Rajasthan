...
function normalize(item, index = 0) {
  ...
  return {
    ...
    image: item?.image || item?.imageUrl || item?.thumbnail || fallback.image, video: item?.video || item?.videoUrl || item?.mediaVideo || "",
...
function safeAdUrl(value) { if (typeof value !== "string" || !value.trim()) return "#"; try { const url = new URL(value, window.location.origin); if (url.protocol !== "http:" && url.protocol !== "https:") return "#"; return url.href; } catch { return "#"; } }function AdSlot({ position = "home_top", className = "" }) {
  ...
  const image = ad.image || ad.imageUrl || ad.banner; const href = safeAdUrl(ad.link);
...
export default function AppProduction() {
  ...
  const [vapidPublicKey, setVapidPublicKey] = useState(BUILD_VAPID_PUBLIC_KEY), [notifyMobile, setNotifyMobile] = useState("");
  useEffect(() => {
  ...
  async function enableNotifications() {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/notifications/subscribe`, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ ...subscription, mobile: notifyMobile }) });
