...
export default function AppProduction() {
  ...
  useEffect(() => { const raw = window.location.hash.match(/^#news-(.+)$/); if (!raw) return; const id = decodeURIComponent(raw[1]); const found = news.find(n => String(n.id) === id || String(n.slug || "") === id); if (found) openArticle(found, false); }, [news]);
