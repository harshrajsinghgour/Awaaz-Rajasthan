...
function formatDate(value) { try { return new Date(value).toLocaleString("hi-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }); } catch { return "अभी"; } }
...
function AdSlot({ position = "home_top", className = "" }) {
  ...
  const image = ad.image || ad.imageUrl || ad.banner; const href = ad.link || "#";
...
export default function AppProduction() {
  ...
  useEffect(() => { if (!API_BASE) { setLoading(false); return; } let cancelled = false; setLoading(true); fetch(`${API_BASE}/api/news?limit=100`, { headers: { Accept: "application/json" } }).then(r => r.ok ? r.json() : Promise.reject()).then(data => { const list = Array.isArray(data) ? data : (data.news || data.data || data.articles || []); if (!cancelled && list.length) setNews(list.map(normalize)); }).catch(() => {}).finally(() => { if (!cancelled) setLoading(false); }); return () => { cancelled = true; }; }, []);
  useEffect(() => { localStorage.setItem("awaaz-bookmarks", JSON.stringify(saved)); }, [saved]);
  useEffect(() => { if (!toast) return; const t = setTimeout(() => setToast(""), 2400); return () => clearTimeout(t); }, [toast]);
  useEffect(() => { const raw = window.location.hash.match(/^#news-(.+)$/); if (!raw) return; const id = decodeURIComponent(raw[1]); const found = news.find(n => String(n.id) === id); if (found) openArticle(found, false); }, [news]);
  ...
  const featured = filtered.find(n => n.featured) || filtered[0] || null, secondary = filtered.filter(n => n.id !== featured?.id).slice(0, 3), latest = filtered.filter(n => n.id !== featured?.id), trending = [...news].sort((a, b) => b.views - a.views).slice(0, 5);
