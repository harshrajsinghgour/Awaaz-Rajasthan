...
function safeAdUrl(value) { if (typeof value !== "string" || !value.trim()) return "#"; try { const url = new URL(value, window.location.origin); if (url.protocol !== "http:" && url.protocol !== "https:") return "#"; return url.href; } catch { return "#"; } }function AdSlot({ position = "home_top", className = "" }) {
  ...
  useEffect(() => { let cancelled=false; fetch(`${API_BASE}/api/categories`,{headers:{Accept:"application/json"}}).then(r=>r.ok?r.json():Promise.reject()).then(d=>{const names=Array.isArray(d?.categories)?d.categories.filter(Boolean):[];if(!cancelled&&names.length)setCategories(["होम",...names.filter(x=>x!=="होम"&&x!=="सभी जिले"),"सभी जिले"]);}).catch(()=>{});return()=>{cancelled=true};}, []);
...
function EpaperPage() {
  const [items,setItems]=useState([]),[date,setDate]=useState(""),[loading,setLoading]=useState(true),[error,setError]=useState("");
  useEffect(()=>{let cancelled=false;setLoading(true);setError("");const q=date?"?date="+encodeURIComponent(date):"";fetch(API_BASE+"/api/epapers"+q,{headers:{Accept:"application/json"}}).then(r=>r.ok?r.json():r.json().then(d=>Promise.reject(new Error(d.message||"ई-पेपर उपलब्ध नहीं है।")))).then(d=>{if(!cancelled)setItems(Array.isArray(d.epapers)?d.epapers:[])}).catch(e=>{if(!cancelled){setItems([]);setError(e.message)}}).finally(()=>{if(!cancelled)setLoading(false)});return()=>{cancelled=true}},[date]);
  return <div className="epaper-page"><header className="epaper-head"><a href="/" className="epaper-brand"><img src="/awaazrajasthan-logo.png" alt="आवाज़ राजस्थान"/><span><b>आवाज़ राजस्थान</b><small>ई-पेपर</small></span></a><a href="/" className="epaper-home">← होम</a></header><main className="epaper-main container"><div className="epaper-title"><span>📰 DAILY EDITION</span><h1>आवाज़ राजस्थान ई-पेपर</h1><p>तारीख चुनें और उस दिन का प्रकाशित ई-पेपर पढ़ें या डाउनलोड करें।</p></div><div className="epaper-filter"><label>📅 ई-पेपर की तारीख<select value={date} onChange={e=>setDate(e.target.value)}><option value="">सभी उपलब्ध तारीखें</option>{items.map(x=><option key={x._id} value={String(x.issueDate).slice(0,10)}>{new Date(x.issueDate).toLocaleDateString("hi-IN")}</option>)}</select></label><label className="epaper-date-input">सीधे तारीख चुनें<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label></div>{loading?<div className="epaper-state">ई-पेपर लोड हो रहा है…</div>:error?<div className="epaper-state">{error}</div>:items.length===0?<div className="epaper-state">इस तारीख के लिए प्रकाशित ई-पेपर उपलब्ध नहीं है।</div>:<div className="epaper-grid">{items.map(x=>{const url=mediaUrl(x.pdf);return <article className="epaper-card" key={x._id}><div className="epaper-preview"><img className="epaper-watermark" src="/awaazrajasthan-logo.png" alt="" aria-hidden="true"/><div className="epaper-preview-icon">📰</div><strong>{x.title||"आज का ई-पेपर"}</strong><span>{new Date(x.issueDate).toLocaleDateString("hi-IN",{day:"numeric",month:"long",year:"numeric"})}</span></div><div className="epaper-card-actions"><button onClick={()=>window.open(url,"_blank","noopener,noreferrer")}>📖 पढ़ें</button><a href={url} target="_blank" rel="noreferrer" download>⬇️ डाउनलोड करें</a></div></article>})}</div>}</main></div>;
}
export default function AppProduction() {
  ...
  const filtered = useMemo(() => { const q = query.trim().toLowerCase(); const source = savedOnly ? [...savedItems, ...news.filter(n => !savedItems.some(s => String(s.id) === String(n.id)))] : news; return source.filter(n => { const categoryMatch = savedOnly ? true : (category === "होम" || n.category === category || n.location === category); const districtMatch = !district || n.location === district || n.category === district || `${n.title} ${n.excerpt}`.includes(district); const textMatch = !q || [n.title, n.excerpt, n.category, n.location, n.author].join(" ").toLowerCase().includes(q); const savedMatch = !savedOnly || saved.includes(String(n.id)); return categoryMatch && districtMatch && textMatch && savedMatch; }); }, [news, savedItems, category, district, query, savedOnly, saved]);
  ...
  function toggleSave(id) { const key = String(id), exists = saved.includes(key); if (exists) { setSaved(prev => prev.filter(x => x !== key)); setSavedItems(prev => prev.filter(x => String(x.id) !== key)); setToast("खबर सेव से हटाई गई"); } else { const item = news.find(n => String(n.id) === key) || (article && String(article.id) === key ? article : null); if (!item) return; setSaved(prev => [key, ...prev]); setSavedItems(prev => [item, ...prev.filter(x => String(x.id) !== key)]); setToast("खबर सेव हो गई"); } }
