...
const upload=multer({storage:multer.diskStorage({destination:(_r,_f,cb)=>cb(null,uploadDir),filename:(_r,f,cb)=>cb(null,`${crypto.randomBytes(12).toString("hex")}${path.extname(f.originalname).toLowerCase()}`)}),limits:{fileSize:Number(process.env.MAX_UPLOAD_MB||8)*1024*1024},fileFilter:(_r,f,cb)=>cb(/^(image\/(jpeg|png|webp|gif)|application\/pdf)$/.test(f.mimetype)?null:new Error("केवल JPG, PNG, WEBP, GIF या PDF फ़ाइल स्वीकार है।"),true)});
...
app.get("/api/news",async(req,res,next)=>{try{const limit=Math.min(Math.max(Number(req.query.limit)||30,1),100),page=Math.max(Number(req.query.page)||1,1),f={status:"published"};if(req.query.category&&req.query.category!=="होम")f.category=String(req.query.category);if(req.query.location)f.location=String(req.query.location);if(req.query.q)f.$text={$search:String(req.query.q)};if(req.query.featured==="true")f.featured=true;const [items,total]=await Promise.all([News.find(f).sort({featured:-1,publishedAt:-1,createdAt:-1}).skip((page-1)*limit).limit(limit).lean(),News.countDocuments(f)]);res.json({news:items,data:items,pagination:{page,limit,total,pages:Math.ceil(total/limit)}});}catch(e){next(e);}});
app.get("/api/news/:id",async(req,res,next)=>{try{const item=mongoose.isValidObjectId(req.params.id)?await News.findOneAndUpdate({_id:req.params.id,status:"published"},{$inc:{views:1}},{new:true}).lean():await News.findOne({slug:req.params.id,status:"published"}).lean();if(!item)return res.status(404).json({message:"News not found"});res.json({news:item,data:item});}catch(e){next(e);}});
...
app.get("/api/ads",async(req,res,next)=>{try{const device=["mobile","desktop"].includes(String(req.query.device))?String(req.query.device):"all",position=String(req.query.position||"home_top");const rows=await Ad.find({position,status:"active",$or:[{device:"all"},{device}]}).sort({createdAt:-1}).lean();res.json({ads:rows.filter(activeAd),data:rows.filter(activeAd)});}catch(e){next(e);}});
app.post("/api/ads/:id/impression",async(req,res)=>{if(!mongoose.isValidObjectId(req.params.id))return res.status(400).json({message:"Invalid ad id"});await Ad.updateOne({_id:req.params.id},{$inc:{impressions:1}});res.status(204).end();});
app.post("/api/ads/:id/click",async(req,res)=>{if(!mongoose.isValidObjectId(req.params.id))return res.status(400).json({message:"Invalid ad id"});await Ad.updateOne({_id:req.params.id},{$inc:{clicks:1}});res.status(204).end();});
app.post("/api/notifications/subscribe",async(req,res,next)=>{try{if(!req.body?.endpoint)return res.status(400).json({message:"Invalid subscription"});await Subscriber.findOneAndUpdate({endpoint:req.body.endpoint},{subscription:req.body,active:true},{upsert:true,new:true,setDefaultsOnInsert:true});res.status(201).json({ok:true});}catch(e){next(e);}});

app.post("/api/admin/login",async(req,res,next)=>{try{const email=String(req.body.email||"").toLowerCase().trim(),password=String(req.body.password||""),a=await Admin.findOne({email});if(!a||!a.active||!(await bcrypt.compare(password,a.passwordHash)))return res.status(401).json({message:"ईमेल या पासवर्ड गलत है।"});a.lastLoginAt=new Date();await a.save();setCookie(res,sign(a));res.json({admin:safe(a)});}catch(e){next(e);}});
...
app.patch("/api/admin/ads/:id",auth,ownerOnly,async(req,res,next)=>{try{const u={};["title","image","link","position","device","status","startDate","endDate"].forEach(k=>{if(req.body?.[k]!==undefined)u[k]=req.body[k]});const ad=await Ad.findByIdAndUpdate(req.params.id,u,{new:true,runValidators:true});if(!ad)return res.status(404).json({message:"Ad not found"});res.json({ad});}catch(e){next(e);}});
...
app.get("/api/admin/ad-analytics",auth,ownerOnly,async(_r,res,next)=>{try{const ads=await Ad.find({}).select("title position device impressions clicks status startDate endDate").sort({clicks:-1,impressions:-1}).lean();const totals=ads.reduce((a,x)=>({impressions:a.impressions+x.impressions,clicks:a.clicks+x.clicks}),{impressions:0,clicks:0});res.json({ads,totals,ctr:totals.impressions?Number((totals.clicks/totals.impressions*100).toFixed(2)):0});}catch(e){next(e);}});
app.get("/api/admin/admins",auth,ownerOnly,async(_r,res,next)=>{try{res.json({admins:await Admin.find({}).select("-passwordHash").sort({createdAt:1}).lean()});}catch(e){next(e);}});
app.post("/api/admin/admins",auth,ownerOnly,async(req,res,next)=>{try{const b=req.body||{},email=String(b.email||"").toLowerCase().trim();if(!email||String(b.password||"").length<10)return res.status(400).json({message:"Email and password (10+ chars) required"});if(b.role==="owner")return res.status(400).json({message:"Owner role is reserved"});if(await Admin.exists({email}))return res.status(409).json({message:"Admin already exists"});const a=await Admin.create({name:b.name,email,passwordHash:await bcrypt.hash(String(b.password),12),role:b.role||"editor",permissions:Array.isArray(b.permissions)?b.permissions:[]});res.status(201).json({admin:safe(a)});}catch(e){next(e);}});
app.patch("/api/admin/admins/:id",auth,ownerOnly,async(req,res,next)=>{try{const u={};["name","permissions","active"].forEach(k=>{if(req.body?.[k]!==undefined)u[k]=req.body[k]});if(req.body?.role!==undefined){if(req.body.role==="owner")return res.status(400).json({message:"Owner role is reserved"});u.role=req.body.role;}if(req.body?.password){if(String(req.body.password).length<10)return res.status(400).json({message:"Password must be at least 10 characters"});u.passwordHash=await bcrypt.hash(String(req.body.password),12);}const a=await Admin.findByIdAndUpdate(req.params.id,u,{new:true,runValidators:true});if(!a)return res.status(404).json({message:"Admin not found"});res.json({admin:safe(a)});}catch(e){next(e);}});
...
app.post("/api/admin/upload",auth,permissions("upload:write"),upload.single("file"),async(req,res)=>{if(!req.file)return res.status(400).json({message:"File required"});res.status(201).json({url:`/uploads/${req.file.filename}`,filename:req.file.filename,mimetype:req.file.mimetype,size:req.file.size});});

async function ensureOwner(){const email=String(process.env.OWNER_EMAIL||"").toLowerCase().trim(),password=String(process.env.OWNER_PASSWORD||"");if(!email||!password){console.warn("OWNER_EMAIL/OWNER_PASSWORD not set; owner bootstrap skipped.");return;}if(await Admin.exists({email}))return;await Admin.create({name:"Owner",email,passwordHash:await bcrypt.hash(password,12),role:"owner",permissions:["news:read","news:write","news:delete","upload:write"]});console.log("Owner account created:",email);}
app.use((err,_req,res,_next)=>{console.error(err);if(err?.code===11000)return res.status(409).json({message:"Duplicate value"});if(err instanceof multer.MulterError)return res.status(400).json({message:err.message});res.status(500).json({message:PROD?"Server error":"Server error: "+err.message});});
