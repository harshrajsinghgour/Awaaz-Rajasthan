...
app.post("/api/admin/ads",auth,ownerOnly,async(req,res,next)=>{try{if(!req.body?.title)return res.status(400).json({message:"Ad title required"});if(req.body.link&&!isHttpUrl(req.body.link))return res.status(400).json({message:"Ad link must be a valid http/https URL"});const ad=await Ad.create({...req.body,createdBy:req.admin._id});res.status(201).json({ad});}catch(e){next(e);}});
app.patch("/api/admin/ads/:id",auth,ownerOnly,async(req,res,next)=>{try{const u={};["title","image","link","position","device","status","startDate","endDate"].forEach(k=>{if(req.body?.[k]!==undefined)u[k]=req.body[k]});if(u.link&&!isHttpUrl(u.link))return res.status(400).json({message:"Ad link must be a valid http/https URL"});const ad=await Ad.findByIdAndUpdate(req.params.id,u,{new:true,runValidators:true});if(!ad)return res.status(404).json({message:"Ad not found"});res.json({ad});}catch(e){next(e);}});
