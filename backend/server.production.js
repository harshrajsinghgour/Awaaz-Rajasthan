...
app.post("/api/payments/razorpay/webhook",express.raw({type:"application/json",limit:"1mb"}),async(req,res)=>{try{if(!RAZORPAY_WEBHOOK_SECRET)return res.status(503).json({message:"Razorpay webhook is not configured"});const signature=String(req.headers["x-razorpay-signature"]||"");const expected=crypto.createHmac("sha256",RAZORPAY_WEBHOOK_SECRET).update(req.body).digest("hex");if(!signature||signature.length!==expected.length||!crypto.timingSafeEqual(Buffer.from(signature),Buffer.from(expected)))return res.status(401).json({message:"Invalid webhook signature"});const event=JSON.parse(req.body.toString("utf8"));if(event.event==="payment.captured"||event.event==="order.paid"){const p=event.payload?.payment?.entity||{},o=event.payload?.order?.entity||{};const orderId=String(p.order_id||o.id||"");const paymentId=String(p.id||"");if(orderId){const b=await AdBooking.findOne({razorpayOrderId:orderId});if(b&&b.paymentStatus!=="paid"){b.paymentStatus="paid";b.status="paid";b.razorpayPaymentId=paymentId;b.updatedAt=new Date();await b.save();}}}if(event.event==="payment.failed"){const p=event.payload?.payment?.entity||{};const orderId=String(p.order_id||"");if(orderId)await AdBooking.updateOne({razorpayOrderId:orderId},{$set:{paymentStatus:"failed",updatedAt:new Date()}});}res.json({ok:true});}catch(e){console.error("RAZORPAY_WEBHOOK_ERROR",e);res.status(500).json({message:"Webhook processing failed"});}});
