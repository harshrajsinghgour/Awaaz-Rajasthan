...
app.patch("/api/admin/admins/:id",auth,ownerOnly,async(req,res,next)=>{try{const u={};let invalidateSessions=false;["name","permissions","active"].forEach(k=>{if(req.body?.[k]!==undefined){u[k]=req.body[k];if(k!=="name")invalidateSessions=true;}});if(req.body?.role!==undefined){if(req.body.role==="owner")return res.status(400).json({message:"Owner role is reserved"});u.role=req.body.role;invalidateSessions=true;}if(req.body?.password){if(String(req.body.password).length<10)return res.status(400).json({message:"Password must be at least 10 characters"});u.passwordHash=await bcrypt.hash(String(req.body.password),12);invalidateSessions=true;}if(invalidateSessions)u.$inc={sessionVersion:1};const a=await Admin.findByIdAndUpdate(req.params.id,u,{new:true,runValidators:true});if(!a)return res.status(404).json({message:"Admin not found"});res.json({admin:safe(a)});}catch(e){next(e);}});
